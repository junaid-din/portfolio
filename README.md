...
* Simulated evolutionary optimisation to recreate a target text sequence (master sequence).
* Used a function to calculate the character-by-character distance between two equal-length strings.
...
* Created N mutated copies (offspring) of this sequence, with each letter having a probability p of changing.
* Chose the best offspring (if better) to continue evolving toward the master sequence.
